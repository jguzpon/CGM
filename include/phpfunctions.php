...
{
	$value = "";
				if($table=="tik_documento" && $field=="Documentos")
	{
		;
		return $value;
	}
				if($table=="tik_restulado" && $field=="Documentos")
	{
		;
		return $value;
	}
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="tik_interno" && $field=="DepartamentoSolicitar")
	{
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="tik_chatau" && $field=="FechaHora")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_chat" && $field=="DeUsuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_chat" && $field=="FechaHora")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_chat" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="tik_memorando" && $field=="Fecha")
	{
		return strftime("%Y-%m-%d");
				...
				if($table=="tik_memorando" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="tik_memorando" && $field=="TipoPapel")
	{
		return Carta;
	}
				if($table=="tik_memorando" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_memorando" && $field=="FechaHora")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_memorando" && $field=="TipoDocumento")
	{
		return I;
	}
				if($table=="tik_memorando" && $field=="CodigoDepto")
	{
		return $_SESSION["CodigoDepto"];
	}
				if($table=="tik_memorando" && $field=="Categoria")
	{
		return Publico;
	}
				if($table=="tik_satisfaccion" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_satisfaccion" && $field=="FechaSistema")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_interno" && $field=="UsuarioSolicitante")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_interno" && $field=="FechaSolicitud")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_interno" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="tik_documento" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_documento" && $field=="FechaSistema")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_restulado" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_restulado" && $field=="FechaSistema")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="zusuarios" && $field=="FechaInicio")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="zusuarios" && $field=="Estado")
	{
		return Activo;
	}
				if($table=="admin_members" && $field=="FechaInicio")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="admin_members" && $field=="Estado")
	{
		return Activo;
	}
				if($table=="admin_users" && $field=="FechaInicio")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="admin_users" && $field=="Estado")
	{
		return Activo;
	}
				if($table=="tik_tipo" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_tipo" && $field=="FechaSistema")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_calendario" && $field=="Fecha")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="tik_calendario" && $field=="TipoRepeticion")
	{
		return 'Unica Vez';
	}
				if($table=="tik_calendario" && $field=="AgregarTicket")
	{
		return No;
	}
				if($table=="tik_calendario" && $field=="TipoActividad")
	{
		return Normal;
	}
				if($table=="tik_calendario" && $field=="Estado")
	{
		return Programado;
	}
				if($table=="tik_calendario" && $field=="UsuarioProgramador")
	{
		return $_SESSION["UserID"];
	}
				if($table=="tik_calendario" && $field=="FechaSistema")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="zdepartamentos" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
				if($table=="zdepartamentos" && $field=="FechaSis")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="vtik_interno" && $field=="UsuarioSolicitante")
	{
		return $_SESSION["UserID"];
	}
				if($table=="vtik_interno" && $field=="FechaAtencion")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="Control de Asistencias" && $field=="FechaSolicitud")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
				...
				if($table=="Control de Asistencias" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="Gráfico de Asistencias" && $field=="FechaSolicitud")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="Gráfico de Asistencias" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="Pendientes por Departamento" && $field=="UsuarioSolicitante")
	{
		return $_SESSION["UserID"];
	}
				if($table=="Pendientes por Departamento" && $field=="FechaSolicitud")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="Pendientes por Departamento" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="Asistencias Realizadas por Fecha" && $field=="UsuarioSolicitante")
	{
		return $_SESSION["UserID"];
	}
				if($table=="Asistencias Realizadas por Fecha" && $field=="FechaInicio")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="Asistencias Realizadas por Fecha" && $field=="Estado")
	{
		return Pendiente;
	}
				if($table=="tik_tipomemo" && $field=="Usuario")
	{
		return $_SESSION["UserID"];
	}
